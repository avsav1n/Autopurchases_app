...
class ShopViewSet(ModelViewSet):
    ...
    - GET /<slug:shop.slug>/export/: Запрос на выгрузку информации о магазине со всем его
        ассортиментом. Для получения файла по окончании подготовки данных необходимо отправить
        GET-запрос на /download/<str:task.id>/. Для выбора типа файла (yaml или json) необходимо
        передать заголовок Accept в запросе (по умолчанию yaml).
    - GET /<slug:shop.slug>/orders/: Получение информации об активных/завершенных заказах
        выбранного магазина.
    - PATCH /<slug:shop.slug>/orders/<int:order.id>/: Изменение статуса выбранного заказа.
...
class CartViewSet(UserFilterMixin, ModelViewSet):
    ...
    - POST /confirm-order/: Заказ всех имеющихся товаров в корзине. Для этого
...
class CeleryTaskView(APIView):
    ...
    Поддерживаемые HTTP-методы:
    - GET /<str:task.id>/: Получение информации о статусе выполнения задачи.
    """
...
class DownloadFileView(APIView):
    ...
    Поддерживаемые HTTP-методы:
    - GET /<str:task.id>/: Получение файла с данными о магазине. Требуемый формат файла (yaml или
        json) можно указать через заголовок Accept в запросе (по умолчанию yaml).
    """
